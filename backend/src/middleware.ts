...
export function requireAdmin(req: Request, res: Response, next: () => void) {
  try {
    ...
    const token = auth.startsWith("Bearer ") ? auth.slice(7) : "";
    if (!token) return res.status(401).json({ error: "缺少 token" });
    const payload = verifyToken(token) as { role?: number; sub: number; tokenVersion?: number; isSuperAdmin?: number };
    assertTokenFresh(payload.sub, payload.tokenVersion);
    if (!payload.isSuperAdmin && (!payload.role || payload.role < 1)) return res.status(403).json({ error: "没有权限" });
    (req as any).uid = payload.sub;
    next();
  } catch (e) {
    res.status(401).json({ error: (e as Error).message });
...
export function requireWikiAdmin(req: Request, res: Response, next: () => void) {
  try {
    ...
    const token = auth.startsWith("Bearer ") ? auth.slice(7) : "";
    if (!token) return res.status(401).json({ error: "缺少 token" });
    const payload = verifyToken(token) as { role?: number; isWikiAdmin?: number; sub: number; tokenVersion?: number; isSuperAdmin?: number };
    assertTokenFresh(payload.sub, payload.tokenVersion);
    if (!payload.isSuperAdmin && (!payload.role || payload.role < 1 || !payload.isWikiAdmin)) return res.status(403).json({ error: "没有权限" });
    (req as any).uid = payload.sub;
    next();
  } catch (e) {
    res.status(401).json({ error: (e as Error).message });
...
export function requireAnnouncementAdmin(req: Request, res: Response, next: () => void) {
  try {
    ...
    const token = auth.startsWith("Bearer ") ? auth.slice(7) : "";
    if (!token) return res.status(401).json({ error: "缺少 token" });
    const payload = verifyToken(token) as { sub: number; isAnnouncementAdmin?: number; role?: number; tokenVersion?: number; isSuperAdmin?: number };
    assertTokenFresh(payload.sub, payload.tokenVersion);
    if (!payload.isSuperAdmin && (!payload.role || payload.role < 1 || !payload.isAnnouncementAdmin)) return res.status(403).json({ error: "没有公告权限" });
    (req as any).uid = payload.sub;
    next();
  } catch (e) {
    res.status(401).json({ error: (e as Error).message });
